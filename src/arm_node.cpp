...
//RMD motor control functions
void rmd_initialize(void);
void rmd_position_control(double joint_1_rad, double joint_2_rad, double joint_3_rad);
void rmd_set_zero(void);

//Can communication object declare
CAN _can("can0", "ttyACM2");
...
void drok4_plugin::JoystickControl(void)
{
    ...
    rmd_position_control(targetRadian[J1],targetRadian[J2],targetRadian[J3]);
...
void JoyCallback(const sensor_msgs::Joy::ConstPtr& msg)
{
    ...
    if (ctrl_mode == 1)
    {
        joy.x = msg->axes[1] * 0.0020;//0.0005;
        joy.y = msg->axes[0] * -0.0020;//-0.0005;
        joy.z = msg->axes[4] * 0.0020;//0.0005;
    }
    else if (ctrl_mode == 2)
    {
...
void rmd_position_control(double joint_1_rad, double joint_2_rad, double joint_3_rad){
  ...
  joint1_deg = (joint_1_rad * R2D);
  joint2_deg = (joint_2_rad * R2D);
  joint3_deg = (joint_3_rad * R2D);

  _can.Joint_1_Pos(joint1_deg);
  usleep(2000);
  _can.Joint_2_Pos(joint2_deg);
  usleep(2000);
 // _can.Joint_3_Pos(joint3_deg);
 // usleep(2000);
}
